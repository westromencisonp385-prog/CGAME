...
- [研究：建立模块与载具引用矩阵](tickets/研究：建立模块与载具引用矩阵.md)：结构证据已整理为 observed / inferred / unknown 矩阵；公式、触发顺序和操控仍待原型验证。
...
- 参考导出中的运行时公式、触发顺序、抽选概率和操控手感；已整理的结构矩阵不能替代未知算法。
...
未解决票据见 [tickets/](tickets/)。当前可处理的开放票据是：
