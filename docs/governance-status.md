...
| 事实面 | 状态 | 证据与边界 |
| --- | --- | --- |
| 代码 | verified-current | `master` 基线为 `62ddeb0`；M0/M1 运行代码、Godot 资源和测试入口均在仓库内。 |
...
| 工作区 | pending | 本次分支需要提交治理配置、wayfinder 路线、研究快照和历史计划；`artifacts/`、`builds/`、`tmp/` 仍由 `.gitignore` 排除。删除候选未自动清理。 |
...
- 保留 `docs/wayfinder/` 的未决票据：引用矩阵、双功能挂点原型、组合预算、首个幻想载具样件和 M0 切片边界仍不能伪造为已决策。
...
2. 先处理 `docs/wayfinder/tickets/研究：建立模块与载具引用矩阵.md`，再处理 `docs/wayfinder/tickets/原型：冻结 M0 双功能挂点与输入语义.md`。
